...
let StartFunc = ({ inTablesCollection, inTo }) => {
    let LocalTypeName = "Sort/restClients/GetEndPoints";
    let LocalTo = inTo;
...
const LocalFuncWriteToHome = ({ inFrom, inTo }) => {
    let LocalFileData = `GET http://localhost:${inFrom}/${CommonSort}`;
...
const LocalFuncWriteToDataOnly = ({ inFrom, inTo }) => {
    ...
    let LocalFileData = `GET http://localhost:${inFrom}/${CommonSort}/${LocalEndPoint}`;
