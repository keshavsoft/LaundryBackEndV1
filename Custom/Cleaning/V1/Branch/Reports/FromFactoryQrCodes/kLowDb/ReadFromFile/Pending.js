...
const jFLocalBranchWideData = ({ inData, inFromDate, inToDate, inBranch }) =>
    inData
        .filter(e => {
            ...
            return itemDate >= inFromDate && itemDate <= inToDate && e.BranchName === inBranch && e.ScanStatus === false;
        })
        .reverse();
...
const LocalDcMergeFunc = ({ inQr, inDc }) => {
    return inQr.map(entryItem => {
        ...
        return {
            ...
            DCDate: isCancelled?.Date || entryItem?.DCDate || ''
        };
    });
};

export { StartFunc };
// let Result = StartFunc({ inBranch: "BranOrdersLBC", inFromDate: "01-04-2025", inToDate: "16-04-2025" }); console.log(Result);
