...
let StartFunc = ({ inFactory, fromDate, toDate }) => {
    ...
    return jFLocalFactoryWideData({ inData: LocalArrayReverseData, inFactory, fromDate, toDate }).reverse();
};
...
const jFLocalFactoryWideData = ({ inData, inFactory, fromDate, toDate }) => {
    ...
    return inData
        .filter(e => {
            const itemDate = e.Date.split('-').reverse().join('-').replace(/\//g, '-');
            return itemDate >= fromDate && itemDate <= toDate && e.FactoryName === inFactory;
        })
        .reverse();
};
...
export { StartFunc };
// StartFunc({ inFactory: "Vizag", fromDate: "09-04-2025", toDate: "09-04-2025" })
